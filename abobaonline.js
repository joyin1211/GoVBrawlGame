...
function startGame() {
    ...
    sideBlock = document.getElementById('sidePrint');
    sideBlock.textContent = 'Your color is not defined!';
...
function drawGuys(x1, y1, x2, y2) {
    ...
    ctx.beginPath();
    ctx.lineWidth = 3;
    ctx.strokeStyle = table[x1][y1].color;
    ctx.moveTo(cx1, cy1);
    ctx.lineTo(cx2, cy2);
    ctx.closePath();
    ctx.stroke();
...
function tableClick(event) {
    ...
    if (table[x][y].constructor.name == "Cell" && table[x][y].type != "dead") {
        ...
        let jsonMessage = JSON.stringify(stepObject);
...
function chatChannel(e) {
    ...
    _chatChannel.onmessage = function (e) {
        ...
        if (info.type == "startNumber") {
            sideBlock = document.getElementById('sidePrint');
            let otherNumber = info.startNumber;
            if (otherNumber > myNumber) {
                myStep = 1;
                sideBlock.textContent = "You are red!";
            } else {
                myStep = 0;
                sideBlock.textContent = "You are blue!";
            }
            console.log(myNumber, otherNumber, myStep);
        }
        if (info.type == "step") {
            let x = info.posx;
            let y = info.posy;
